...
## 进行中
- 队标功能待用户手动重建 Docker 镜像生效：logo 提取代码已 :8083 隔离实例实证正确（`/api/teams/6667` profile.logo + recent_matches team1/team2_logo 全为真实 imgix URL，非 placeholder），仅缺镜像重建。用户需执行 `docker build -t hltv-data:test .` + 重启容器 hltv-test，浏览器 :8082 验证队标显示（TeamLogo 组件 + 后端 logo 提取/持久化/migration v3 代码已就绪；当前容器跑 15:37 旧镜像不含 logo 代码）；选手对比 nickname 已测试通过
