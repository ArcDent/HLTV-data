...
## 最近操作
- 2026-07-01：前端样式优化 — 金色背景去除 + 中文字体增强 + 背景层次提升。金色渐变背景（rank badge、achievements）全部改为透明背景 + 金色文字 + 细边框，提升深色主题质感；添加 Noto Sans SC 中文字体栈，解决中文回退问题；body 背景从纯色改为径向渐变，增强空间深度；tokens.css 更新阴影系统增加 inset 高光。已推送 origin/main（5 文件：tokens.css/base.css/TeamDetail.tsx/TeamComparison.tsx/PlayerDetail.tsx）
- 2026-07-01：新闻详情"先空内容再刷新才有内容"Bug 修复 — 用户反馈点击新闻/归档详情先显示空内容，退出强制刷新页面后再点才有内容，而非走加载进度条→内容。systematic-debugging Phase 1 根因：GetNewsArticleCached (facade.go:137) SQLite 缓存层把 BatchUpsertNews 写入的 list-item 行（body_text NULL）误判为有效 cache hit，直接返回空 body article 且仅触发后台 async refreshNewsArticle；后台完成后写完整 body 入缓存 + broadcast SSE 'news'，useSSE 触发 News.tsx fetchNews 重载 list（setData(null)），用户关闭再点第二次命中内存缓存完整 body → 三阶段现象（空内容→list 重载→再点有内容）完整解释。Phase 4 修复：SQLite 查询加 `article.BodyText != ""` 条件，body_text 空的行 fall through 到同步 scrapeNewsArticle，第一次打开即 spinner → 完整内容。验证：go build ./... 通过；Docker hltv-data:test 重建 + 容器 hltv-test 重启 :8082；/api/health ok + /api/status 6/6 端点 ok；行为验证：/api/news?limit=3 取 list link /news/45026/sources-saw-locked-in-talks-with-former-quartet（list item body_text 空），调 /api/news/article?url=... 返回 body_text length 1844 + partial:false（修复前此场景返回空 body）；交用户手动测试
...
## 进行中
- 队标 + 归档新闻 link 修复已随镜像重建（hltv-data:test）+ 容器 hltv-test 重启生效于 :8082：logo 提取/持久化/migration v3 代码 + NormalizeNews Closest("a") 修复 + useSSE 收敛均已编入运行镜像；/api/health ok；/api/news?limit=30 返回 30 items / 30 unique links（归档新闻 link 重复 bug 已实证修复）。待用户浏览器 http://localhost:8082 手动验证：(1) 队标显示（TeamLogo 组件，队伍页 /matches /results）；(2) 归档新闻 tab 点击任意新闻详情，确认各自跳转正确新闻而非同一篇；选手对比 nickname 已测试通过；(3) 新闻/归档详情首次点击即走加载进度条显示内容（不再先空内容再刷新，facade BodyText != "" fall through 同步 scrape 修复已编入镜像）
