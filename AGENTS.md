...
## 进行中
- 用户手动测试 Docker 容器 hltv-test（http://localhost:8082），含本次瘦身收敛 + 前端 3 Bug + Round 2 后端根因改动；测完反馈后停止容器（docker stop hltv-test && docker rm hltv-test）
